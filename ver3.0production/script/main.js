...
gameRef.once('value', function(dataSnapshot) {
  ...
  $scope.gameContainer = {
    ...
    FBgameClock: $scope.gameClock,
    FBwinScene:$scope.winScene
  };
...
$scope.updatePlayer = function(){
  ...
  chatRef.push({userName:$scope.currentPlayer.userName, userComment:("joined the room.")});
...
$scope.joinGame = function(){
  if($scope.gameContainer.FBgameManager.activePlayer>=2){
    ...
    $scope.gameContainer.gameClock = Date.now();
  }
}
// THE JUDGE
var judgeGame = function (obj){
  ...
  for (var i=0; i<=winningMoves.length;i++) {
    ...
    if (result){
      ...
      $scope.gameContainer.FBwinScene = i;
      break;
    };
  }
  return result;
...
$scope.chooseTile = function(thisTile){
  // Time Between plays
  var deltaTime = 0;
  // "Simplify" logic to make sure only current player can play and when it's their turn
  var isFirstIDmatch = false;
  isFirstIDmatch = $scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.firstPlayer].playerID==$scope.playerID;
  var isSecondIDmatch = false;
  isSecondIDmatch = $scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.secondPlayer].playerID==$scope.playerID;
  var isFirstPlayerTurn = false;
  isFirstPlayerTurn = isFirstIDmatch == (($scope.gameContainer.FBcount % 2) == 0);
  var isSecondPlayerTurn = false;
  isSecondPlayerTurn = isSecondIDmatch == (($scope.gameContainer.FBcount % 2) == 1);
  if (((thisTile.status=='0')&&($scope.gameContainer.FBgameManager.activePlayer==2)) && (isFirstPlayerTurn || isSecondPlayerTurn)) {
    ...
    if (($scope.gameContainer.FBcount % 2) == 0) {
      ...
      // Update time as soon as tile is played.
      deltaTime = Date.now()-$scope.gameContainer.gameClock;
      $scope.gameContainer.gameClock = Date.now();
      ...
      if ($scope.gameContainer.FBcount>=4){
        console.log();
        if (judgeGame($scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.firstPlayer].board)) {
          announcer($scope.gameContainer.FBgameManager.firstPlayer);
        }
    ...
    // SECOND MOVE
      ...
      // Let's find out the winner after every tile click - And only check after the 5th turn
      if ($scope.gameContainer.FBcount>=4){
        if (judgeGame($scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.secondPlayer].board)) {
          announcer($scope.gameContainer.FBgameManager.secondPlayer);
        }
        else if ($scope.gameContainer.FBcount == 8) {
          tie = true;
          $scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.secondPlayer].waitTime <= $scope.gameContainer.FBplayer[$scope.gameContainer.FBgameManager.secondPlayer].waitTime ?
      ...
      // Reset Poll Numner
      for (var i = 0; i < $scope.gameContainer.FBgameManager.pollBoard.length; i++) {
        $scope.gameContainer.FBgameManager.pollBoard[i].pollNum = 0;
      }
  }
...
var updateBoard = function () {
  if (!tie){
    var winMove = winningMoves[$scope.gameContainer.FBwinScene].split('');
...
$scope.reloadGame = function(){
  ...
  tie = false;
  $scope.gameContainer.FBwinScene = "";
...
$scope.hardResetGame = function(){
  ...
  $scope.gameContainer.FBplayer[0].score = 0;
  $scope.gameContainer.FBplayer[1].score = 0;
  $scope.gameContainer.FBplayer[0].name = "waiting...";
  $scope.gameContainer.FBplayer[1].name = "waiting...";
  $scope.gameContainer.FBplayer[0].isNext = true;
  $scope.gameContainer.FBplayer[1].isNext = false;
