  ...
  // 
  var gameSyncRef = new Firebase("https://wdi2014.firebaseio.com") ;
  
  // Firebase connection - AngularFire reference to database
  var gameSync = $firebase(gameSyncRef);

  // Download data into local object
  var gameBoardSync = gameSync.$asObject();

  // Add 3 way data binding
  gameBoardSync.$bindTo($scope, "gameBoard")
  ...
  $scope.player = [
  {name:"",score:0,time:0, tilechoice:"X", board:["0","0","0","0","0","0","0","0","0"], turnWinner:false},
  {name:"",score:0,time:0, tilechoice:"O", board:["0","0","0","0","0","0","0","0","0"], turnWinner:false}
  ];
    ...
    for (var i = 0; i < winningMoves.length; i++) {
      ...
      for (var j = 0; j < winMove.length; j++) {
        ...
        if (winPoint == 3) {
          ...
          // console.log("breakyet?" +"i="+i);

          break;
        }
        //
        // console.log("wP="+winPoint+" count="+$scope.count+" tie="+tie+"result= "+result);
        // console.log("the comparison - "+!result && $scope.count==9+"i= "+i+"j= "+j);
        if (!result && $scope.count==9){
          tie=true;
        };
      };
    };
  ...
  $scope.chooseTile = function(thisTile){
    ...
    if (judgeGame($scope.player[0].board)) {
      announcer(firstTile,0); //Announce the winner
      $scope.player[0].score++; //Update score
    }
    if (judgeGame($scope.player[1].board)) {
      announcer(secondTile,1);
      $scope.player[1].score++;
    };
    console.log($scope.count+" tie= "+tie);
    if (tie) {
      announcer("TIE","");
    };
    }
  ...
  var announcer = function(result,userID){
    if (result=="TIE") {
      alert("TIE GAME!" + tie);
    };
    if (result == "X") {
  ...
  $scope.reloadGame = function(){
    ...
    $scope.player[0].turnWinner =false;
    $scope.player[1].turnWinner =false;
    $scope.gameBoard = [
    {name:"c0",status:"0"},
    {name:"c1",status:"0"},
    {name:"c2",status:"0"},
